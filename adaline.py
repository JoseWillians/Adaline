...
if 'treinado' not in st.session_state:
    ...
    st.session_state.pesos = np.zeros(3)
    st.session_state.historico = []
    st.session_state.epocas_totais = 0
    st.session_state.pesos_iniciais = np.zeros(3) # Para mostrarmos como a IA "nasceu"
...
if st.button("Iniciar Treinamento", type="primary"):
    ...
    st.session_state.pesos = w
    st.session_state.historico = historico_eqm
    st.session_state.epocas_totais = epoca
...
if st.session_state.treinado:
    ...
    w_init = st.session_state.pesos_iniciais
    w_final = st.session_state.pesos
    
    # Mostramos o antes e o depois para ficar bem didático!
    st.write(f"**Pesos Iniciais (Como a IA nasceu):** `[{w_init[0]:.4f}, {w_init[1]:.4f}, {w_init[2]:.4f}]`")
    st.write(f"**Pesos Finais Ajustados (Após aprender):** `[{w_final[0]:.4f}, {w_final[1]:.4f}, {w_final[2]:.4f}]`")
...
if st.session_state.treinado:
    ...
    if st.button("🧪 Testar Rede"):
        entrada_teste = np.array([-1, x1_val, x2_val])
        u = np.dot(st.session_state.pesos, entrada_teste)
